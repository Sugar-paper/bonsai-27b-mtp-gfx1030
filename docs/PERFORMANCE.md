...
**⚠️ 测量陷阱（2026-09-24 实测踩坑）**：无审查包首轮 llama-bench 曾测得 pp512=251 / tg128=3.81
（慢 12 倍）—— 根因是**之前测试的 llama-kvmem-server 进程没退干净、仍占用 GPU 显存**，导致权重
放不进 VRAM、部分层落 CPU。清掉残留进程（`taskkill /F /IM llama-kvmem-server.exe` 等）后背靠背
重测：无审查 594.50/580.72/45.58 vs 官方 589.02/577.79/45.51 —— **两包速度完全一致**。
三个启动器默认端口都是 11234，先起的服务没关就双击另一个，必然踩这个坑。

**三包干净对比**（llama-bench，同条件背靠背，-ngl 99，r=2，2026-09-24）：

| 权重包 | 量化 | 大小 | pp512 | pp4096 | tg128 |
|---|---|---:|---:|---:|---:|
| ProCreations 官方 Q8_0 | PQ2_0 · 2.13 bpw | 7.12 GiB | 589.0 | 577.8 | **45.5** |
| BoldingBuilds 无审查 | PQ2_0 · 2.13 bpw | 7.12 GiB | 594.5 | 580.7 | **45.6** |
| ISTA-DASLab Qwen3.8-27B | IQ3_S · 3.44 bpw | 11.28 GiB | 488.1 | 477.4 | 29.8 |

> 结论：**两个 Bonsai 包（官方/无审查）速度完全一致**（张量结构相同）；ISTA（Qwen3.8-27B 本体，
> IQ3_S 3.44 bpw、11.3 GiB）反而最慢——量化位宽更大 = 每 token 读取带宽更大。
> 「无审查包慢」的印象全部来自残留进程污染的测量。
...
**新 budget 98304（2026-09-24，默认）**：

| prompt tokens | prefill (pp) | 生成 | 备注 |
|---:|---:|---:|---|
| **199,950（≈256K 上下文）** | **242.97 t/s** | 33.9 t/s（70 tok 样本，思考计入） | 答案正确、无 OOM；pp 比旧 budget 的 147K（225.5）还高 **+7.7%** |

- KVMem 全程生效：常驻池 53,248 cells，长 prompt 期间按块 offload 到主机内存（`need_offload` 触发）
- 生成阶段无抖动，说明分层 KV 的取回路径没有落在关键路径上
- 256K 上下文（262144）**已验证可用**：~200K token 长 prompt 全量 prefill + 生成，均正常
...
| 视觉 | ✅ 官方投影器 | ✅ **复用官方 mmproj**（两包张量逐项相同；实测 32 s 冷启动、识别正确） |

> 两个包的张量集合逐项相同（866 张量、`block_count = 65`、零 shape/type 差异），
> 因此**速度差异落在噪声内**，区别只在权重行为（消融 vs 官方）；投影器共用官方 mmproj。
